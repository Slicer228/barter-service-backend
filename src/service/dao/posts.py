...
class Posts:
                    ...
                    try:
                        ...
                        if post.photos:
                            for photo in post.photos:
                                stmt = insert(Post_photos).values(post_photo=bytes(photo.post_photo,'utf8'),post_photo_name=photo.post_photo_name, post_id=post_id)
                                await session.execute(stmt)
    ...
    async def get(post_id: int | list[int], user_id: int | list[int] | None = None):
        ...
        if isinstance(post_id, list):
            ...
            if user_id:
                async with async_session_maker() as session:
                    for p,u in zip(post_id,user_id):
                        ...
                        stmtPP = select(Post_photos).where(Post_photos.post_id == p)
            ...
            stmtC = select(Categories.category_id).join(Post_categories.category_names).where(Post_categories.post_id == post_id).order_by(Post_categories.category_type)
            async with async_session_maker() as session:
                post = await session.execute(stmtP)
                post = post.scalars().first()
                photos = await session.execute(stmtPP)
